...
Infisical has two major code-bases: one for the platform code, and one for SDKs. The contribution process has some differences between the two, therefore we have split the documentation into two sections:
